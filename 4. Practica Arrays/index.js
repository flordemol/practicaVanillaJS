const inventors = [
  ...
  // 1. Filtrar la lista de inventores para aquellos que nacieron en los 1500's
  ...
  // 3. Ordene los inventores por fecha de nacimiento, de mayor a menor

        const sortBirthdate = inventors.sort((inventorA, inventorB) => (inventorA.year > inventorB.year) ? 1 : -1);
  ...
  // 5. Ordenar los inventores por años vividos
        ...
        const orderYears = inventors.sort((inventorA, inventorB) => {
            return ((inventorA.passed - inventorA.year) - (inventorB.passed - inventorB.year));
        })
        console.table(orderYears);
  ...
  // https://en.wikipedia.org/wiki/Category:Boulevards_in_Paris
      ...
      /*  
        ...
        const links = [...categories.querySelectorAll('a')]; // Pasar de NodeList a Array
        const list = links
...
*/
  ...
  const transport = data.reduce((objeto, item) => {
      if(!objeto[item]){
        objeto[item] = 0;
      }
      objeto[item]++;
